...
```{r}
pf2 <- pf |> select(species, island, gender, BMI_category) |> as.data.frame()
```
...
```{r}
pf2$BMI_category <- factor(pf2$BMI_category, levels = c("Underweight", "Normal", "Overweight", "Obese"))

# Create grouped bar chart
grouped_bar_chart <- ggplot(pf2, aes(x = species, fill = BMI_category)) +
  geom_bar(position = "dodge", stat = "count") +
  labs(title = "Grouped Bar Chart of Species by BMI Category",
       x = "Species",
       y = "Count",
       fill = "BMI Category") +
  scale_fill_manual(values = c("#440154FF", "#414487FF", "#2A788EFF", "#7AD151FF"),
...
```{r}
grouped_bar_chart2 <- ggplot(pf2, aes(fill = species, x = BMI_category)) +
  geom_bar(position = "dodge", stat = "count") +
  labs(title = "Grouped Bar Chart of Species by BMI Category",
       fill = "Species",
       y = "Count",
